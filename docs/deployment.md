...
2. Point the service at `apps/backend`.
...
2. Build from `apps/backend/Dockerfile`.
...
- run `pnpm --filter @rlaas/backend prisma:migrate:deploy` manually once if needed
...
### 5. Static bootstrap credentials leak into a shared environment

Symptoms:

- shared accounts or static API keys appear in a public deployment

Typical fixes:

- avoid running `db:seed` in public production
- avoid baking credentials into deployment scripts or docs
